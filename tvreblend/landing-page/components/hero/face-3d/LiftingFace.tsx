...
const liftingFragmentShader = `
  ...
  void main() {
    ...
    // ========================================
    // Level 3: Density Grid Effect (밀도망/골격 강화)
    // ========================================
    float liftLevel = max(uLiftIntensity, 0.0);
    
    // Level 3(intensity ~3.0) 근처에서 나타나는 효과
    if (liftLevel > 2.0) {
      // 3D 좌표 기반 그리드 생성
      float scale = 30.0;
      float gridLineThickness = 0.03; // 라인 두께
      
      // X, Y, Z 각각에 대한 그리드 패턴
      float gridX = step(1.0 - gridLineThickness, fract(vPosition.x * scale + uTime * 0.1));
      float gridY = step(1.0 - gridLineThickness, fract(vPosition.y * scale));
      float gridZ = step(1.0 - gridLineThickness, fract(vPosition.z * scale));
      
      // 육각형/Tech 느낌을 위한 조합
      float gridPattern = max(gridX, max(gridY, gridZ));
      
      // 얼굴 외곽/리프팅 부위에만 강하게 적용
      float densityMask = affectedArea * smoothstep(2.0, 3.0, liftLevel);
      
      // 은은하게 빛나는 효과
      vec3 gridGlow = uGridColor * gridPattern * densityMask * 0.6;
      
      // 기존 색상에 더하기 (Additive)
      litColor += gridGlow;
      
      // 베이스에도 약간의 틴트 추가 (단단한 느낌)
      litColor = mix(litColor, litColor * vec3(0.9, 0.9, 1.0), densityMask * 0.3);
    }
    ...
    () =>
      new THREE.ShaderMaterial({
        ...
        uniforms: {
          ...
          uGridColor: { value: new THREE.Color("#e0c7ff") }, // 바이올렛/미스틱 그리드 (Indigo 테마와 조화)
        },
